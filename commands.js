...
async function handleCreateBusinessCommand(interaction) {
  ...
  if (userData.business) {
    return interaction.reply({ content: "⚠️ You already own a business! Use `/business_info` or `/upgrade_business`.", ephemeral: true });
  }
  ...
  if (userData.balance < typeData.cost) {
    return interaction.reply({ content: `❌ You need ¥${typeData.cost.toLocaleString()} to start this business.`, ephemeral: true });
  }

  userData.balance -= typeData.cost;
  userData.business = {
    name,
    type,
    level: 1,
    income: typeData.income,
    incomeRate: typeData.incomeRate,
    upgradeCost: Math.floor(typeData.cost * 1.5),
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`You created **${name}** (${type})`)
    .addFields(
      { name: "Level", value: "1", inline: true },
      { name: "Income", value: `¥${typeData.income}/min`, inline: true },
      { name: "Next Upgrade Cost", value: `¥${userData.business.upgradeCost}`, inline: true },
    );

  return interaction.reply({ embeds: [embed], ephemeral: true });
}

async function handleCollectIncomeCommand(interaction) {
  const userId = interaction.user.id;
  await database.ensureUser(userId);
  const userData = await database.getUserData(userId);

  if (!userData.business) {
    return interaction.reply({ content: "❌ You don’t own a business yet.", ephemeral: true });
  }

  const now = Date.now();
  const elapsedMinutes = Math.floor((now - userData.business.lastCollected) / 60000);
  if (elapsedMinutes <= 0) {
    return interaction.reply({ content: "⏳ No income yet. Wait a bit longer!", ephemeral: true });
  }

  const incomeEarned = elapsedMinutes * userData.business.income;
  userData.balance += incomeEarned;
  userData.business.lastCollected = now;

  await database.saveUserData(userId, userData);

  return interaction.reply({
    content: `💰 You collected **¥${incomeEarned.toLocaleString()}** from your **${userData.business.name}**!`,
    ephemeral: true,
  });
}

async function handleUpgradeBusinessCommand(interaction) {
  const userId = interaction.user.id;
  await database.ensureUser(userId);
  const userData = await database.getUserData(userId);
  ...
  if (userData.balance < userData.business.upgradeCost) {
    return interaction.reply({ content: `❌ You need ¥${userData.business.upgradeCost.toLocaleString()} to upgrade.`, ephemeral: true });
  }

  userData.balance -= userData.business.upgradeCost;
  userData.business.level++;
  userData.business.income = Math.floor(userData.business.income * INCOME_MULTIPLIER);
  userData.business.upgradeCost = Math.floor(userData.business.upgradeCost * UPGRADE_COST_MULTIPLIER);

  await database.saveUserData(userId, userData);

  return interaction.reply({
    content: `📈 Your **${userData.business.name}** is now level ${userData.business.level}! Income: ¥${userData.business.income}/min.`,
    ephemeral: true,
  });
}

async function handleBusinessInfoCommand(interaction) {
  const userId = interaction.user.id;
  await database.ensureUser(userId);
  const userData = await database.getUserData(userId);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Income", value: `¥${biz.income}/min`, inline: true },
      { name: "Upgrade Cost", value: `¥${biz.upgradeCost}`, inline: true },
    );
...
module.exports = {
  ...
  handleCreateBusinessCommand,
  handleCollectIncomeCommand,
  handleUpgradeBusinessCommand,
  handleBusinessInfoCommand,
};
